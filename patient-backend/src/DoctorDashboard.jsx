      ...
      <div className="shell">
        <div className="card wide">
          <div className="row-between">
            <h1>{selected.Name}</h1>
            <button className="ghost" onClick={() => setSelected(null)}>← Back to patient list</button>
          </div>
          ...
          {activeTab === 'overview' && (
            <>
              {hasAllergyAlert && (
                <div className="emergency-alert" style={{ marginBottom: '16px' }}>
              ...
              {!overviewLoading && overview.urgentCount > 0 && (
                <div className="emergency-alert" style={{ marginBottom: '16px' }}>
                  <strong>
                    {overview.urgentCount} urgent finding{overview.urgentCount > 1 ? 's' : ''} in
                    uploaded documents
                  </strong>
                  <p>
                    <button className="ghost" onClick={() => setActiveTab('documents')}>
                      Open Documents tab
                    </button>
                  </p>
                </div>
              ...
              <div className="vitals-grid">
                ...
                <div className="vital-stat">
                  ...
                  <strong>{selected.Age}, {selected.Gender}</strong>
                </div>
                <div className="vital-stat">
                  <span className="muted">Last Visit</span>
                  <strong>{overviewLoading ? '…' : overview.latestVisit?.Visit_Date || 'No visits yet'}</strong>
                </div>
                <div className="vital-stat">
          ...
          {activeTab === 'documents' && (
            <div className="history-section" style={{ marginTop: 0, paddingTop: 0, border: 'none' }}>
              <PatientDocuments patientId={selected.id} />
            </div>
          )}

          {activeTab === 'history' && <PatientHistory patient={selected} readOnly />}
        </div>
      </div>
    )
  }
